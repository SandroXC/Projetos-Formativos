...
export const postCriar = async (req, res) => {
  const { nome, diretor, img, descricao, ano, iframe } = req.body;
  try {
    // await connection.query(`INSERT INTO filmes (nome, diretor, img, descrição, ano, iframe) VALUES('${nome}', '${diretor}', '${img}', ${duracao}, '${ano}', '${iframe}')`)
    if (!nome || !diretor || !img || !descricao || !ano || !iframe) {
      res.send("Todos os campos são obrigatórios!");
    } else {
      await filmes.create({ nome, diretor, img, descricao, ano, iframe });
